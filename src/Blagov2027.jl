function makeHypSetup(::Blagov2027_type)
    return hypChan2020()
end
...
@doc raw"""
    Main function for Blagov2027
"""
function beavar(::Blagov2027_type, set_struct, hyp_struct, data_struct)
    println("Hello Blagov2027")
    @unpack dataHF_tab,dataLF_tab, var_list = data_struct
    store_β, store_Σt_inv, store_YY, M_zsp, z_vec, Sm_bit, freq_mix_tp, store_Σt, store_h, store_s2_h, store_ρ, store_σ_h2, store_eh, M_inter_agg, fdatesHF, fdatesLF = Blagov2027(dataHF_tab,dataLF_tab,var_list,set_struct,hyp_struct)    
    out_struct = VAROutput_Blagov2027(store_β, store_Σt_inv, store_YY, M_zsp, z_vec, Sm_bit, freq_mix_tp, store_Σt, store_h, store_s2_h, store_ρ, store_σ_h2, store_eh, M_inter_agg, fdatesHF, fdatesLF)
    return out_struct
end
...
@doc raw"""
    dataBlagov2027(data_HF::TimeArray,data_LF::TimeArray,prior_RW::Int,var_list::Array{Symbol,1})

generate a dataset strcture for use with Blagov2027 model
...
"""
@with_kw struct dataBlagov2027{Ttyp <: AbstractFloat, Ntyp, Dtyp, Atyp <: AbstractArray{Ttyp, Ntyp}} <: BVARmodelDataSetup
    dataHF_tab::TimeArray{Ttyp,Ntyp,Dtyp,Atyp}                       # data for the high-frequency variables
    dataLF_tab::TimeArray{Ttyp,Ntyp,Dtyp,Atyp}                       # data for the low-frequency variables
...
"""
function makeDataSetup(::Blagov2027_type,dataHF_tab::TimeArray, dataLF_tab::TimeArray; var_list =  [colnames(dataHF_tab); colnames(dataLF_tab)])
    return dataBlagov2027(dataHF_tab, dataLF_tab, var_list)
end



@doc raw"""
     BEAVARs.Blagov2027(dataHF_tab,dataLF_tab,varList,varSetup,hypSetup)
...
# Description
The function implements the mixed-frequency BVAR model as in Blagov et al. (2027).
# Reference
...
"""
function Blagov2027(dataHF_tab::TimeArray{Typ,Ntyp,Dtyp,Atyp},dataLF_tab::TimeArray{Typ,Ntyp,Dtyp,Atyp},varOrder::Array{Symbol,1},varSetup::BVARmodelSetup,hypSetup::BVARmodelHypSetup) where {Typ <: AbstractFloat, Ntyp, Dtyp, Atyp <: AbstractArray{Typ, Ntyp}}
    ...
    cB,H_B,Σ_invsp  = BEAVARs.Blagov2027_updCPZ!(cB,H_B,Σ_invsp,B_draw,structB_draw,Σt_inv,Y0,cB_b0_LI,p,n,H_B_CI,strctBdraw_LI,Σt_LI);
    YYt             = BEAVARs.Blagov2027_draw_wz!(YYt,longyo,cB,Σ_invsp,Sm_bit,Smsp,Sosp,nm,MOiM,MOiz,Gm,Go,H_B,GΣ,Kym);
    
        
    # Initialize matrices for updating the parameter draws from CPZ_iniv  
    # ------------------------------------
    Y, X, T, k, sigmaP, S_0, Σ, A_0, V_Ainv, v_0, H_ρ,h,eh,Ωinv, dg_ind_Ωinv, VAinvDA0, AVAinvA, intercept   = BEAVARs.Blagov2027_initcsv(YY,p,hypSetup,prior_RW);
    ...
    @showprogress for ii in 1:ndraws
        ...
        cB,H_B,Σ_invsp  = BEAVARs.Blagov2027_updCPZ!(cB,H_B,Σ_invsp,B_draw,structB_draw,Σt_inv,Y0,cB_b0_LI,p,n,H_B_CI,strctBdraw_LI,Σt_LI);
        Σ_invsp         = BEAVARs.Blagov2027_updCPZcsv!(Σ_invsp,p,n,Tf,Ωinv);
        YYt             = BEAVARs.Blagov2027_draw_wz!(YYt,longyo,cB,Σ_invsp,Sm_bit,Smsp,Sosp,nm,MOiM,MOiz,Gm,Go,H_B,GΣ,Kym);
...
#------------------------------
# Output structure
@with_kw struct VAROutput_Blagov2027 <: BVARmodelOutput
    store_β::Array{}        # 
    store_Σt_inv::Array{}        # 
    store_YY::Array{}
    M_zsp::Array{} 
...
"""
function Blagov2027_initcsv(YY,p,hypSetup,prior_RW)
...
"""
function Blagov2027_draw_wz!(YYt,longyo,cB,Σ_invsp,Sm_bit,Smsp,Sosp,nm,MOiM,MOiz,Gm,Go,H_B,GΣ,Kym;nmdraws=10)
...
function Blagov2027_updCPZ!(cB,H_B,Σ_invsp,B_draw,structB_draw,Σt_inv,Y0,cB_b0_LI,p,n,H_B_CI,strctBdraw_LI,Σt_LI)
    # updating cB
...
function Blagov2027_updCPZcsv!(Σ_invsp,p,n,Tf,Ωinv)    
...
@doc raw"""
    Magg = Blagov2027_createMagg(dataHF_tab,freq_mix)
...
"""
function Blagov2027_createMagg(timeHF,freq_mix)
...
@doc raw"""
    lf_mat, lf_mat_med = Blagov2027_hf2lf(out_struct,Magg,var_name::Symbol)

Convert high-frequency draws to low-frequency draws.

# Arguments
    out_struct: output structure from the Bayesian VAR model
    Magg:       matrix with the aggregation restrictions, output from Blagov2027_createMagg
    var_name:  Symbol for the variable that should be aggregated to lower frequency

See also Blagov2027_createMagg
"""
function Blagov2027_hf2lf(out_struct,Magg,var_name::Symbol)
...
#--------------------------------------
# Forecast Block for Blagov2027
@doc raw"""

"""
function forecast(VAROutput::VAROutput_Blagov2027,VARSetup::BVARmodelSetup,data_struct::BVARmodelDataSetup)
